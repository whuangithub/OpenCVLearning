...
int Drawing_Random_Lines( Mat image, char* window_name, RNG rng )
{
	...
	for( int i = 0; i < NUMBER; i++ )  // NUMBER lines
	{
		...
		line( image, pt1, pt2, randomColor(rng), rng.uniform(1, 10), 8 );  // randomColor is defined above, random thickness
		imshow( window_name, image );
		if( waitKey( DELAY ) >= 0 )
			{ return -1; }
	}
...
int Displaying_Random_Text( Mat image, char* window_name, RNG rng )
{
	...
	for ( int i = 1; i < NUMBER; i++ )
	{
		...
		putText( image, "Testing text rendering", org, // location of the bottom-left corner
					 rng.uniform(0,8),                       // font type
					rng.uniform(0,100)*0.05+0.1,            // scale of the font
					randomColor(rng), rng.uniform(1, 10),   // thickness
					 lineType);
