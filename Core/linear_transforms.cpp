 ...
 * @author OpenCV team
 * @some comments added by whuangithub
 */
...
int main( int argc, char** argv )
{
		...
		// The keys parameter is a string containing several blocks, each one is enclosed in curly braces and describes one argument.
		// Each argument contains three parts separated by the | symbol.
		//{	argument names|default value(can be empty)(can be empty)|help message(can be empty)}
		//      ^         :to mark argument as positional, prefix it with the @ symbol
				...
				for( int x = 0; x < image.cols; x++ ) {
						for( int c = 0; c < image.channels(); c++ ) {
								new_image.at<Vec3b>(y,x)[c] =
									// use saturate_cast to make sure the value is valid
